            ...
            {post.post_ministers?.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                    {post.post_ministers.map((pm: any) => (
                        <Link key={pm.minister_id} href={`/ministers/${pm.minister_id}`} className="rounded-full bg-[#1e3a5f]/5 px-3 py-1 text-xs font-medium text-[#1e3a5f] transition hover:bg-[#1e3a5f]/10">
            ...
            {post.source_url && (
                <div className="mt-10 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
                    <p className="text-xs text-neutral-500">
                        Source: <a href={post.source_url} target="_blank" rel="noopener noreferrer" className="text-[#1e3a5f] underline">{post.source_url}</a>
                    </p>
                </div>
            )}
