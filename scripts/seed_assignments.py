...
def seed_assignments():
    ...
    manifesto_items = (
        db.table("manifesto_items").select("id, category, source_id").execute().data
    )
    ...
    for item in manifesto_items:
        ...
        if source_id == "bp-097":  # Border Security
            minister_names = list(
                set(minister_names + ["Sudan Gurung", "Shishir Khanal"])
            )
        ...
        for name in minister_names:
            if name in minister_map:
                assignments.append(
                    {"minister_id": minister_map[name], "manifesto_item_id": item["id"]}
                )

    # 5. Insert in batches
    if assignments:
        # Clear existing
        db.table("minister_manifesto_assignments").delete().neq(
            "minister_id", "00000000-0000-0000-0000-000000000000"
        ).execute()
